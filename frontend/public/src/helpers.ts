...
export async function isOk(response: Response) {
    if (!response.ok) {
        ...
        else if (response.headers.get("content-type")?.includes("video/quicktime")) {
            return URL.createObjectURL(await response.blob());
        }
        else {
...
export function percentToFrame(percent: number) {
    ...
    return Math.floor(percent / 100 * g.MODEL3D.anim.maxFrame);
}

export function frameToPercent(frame: number) {
    frame = clamp(frame, 0, g.MODEL3D.anim.maxFrame);
    return frame / g.MODEL3D.anim.maxFrame * 100;
}
...
export function isForClientMode() {
    // check url param forclient=true
    const urlParams = new URLSearchParams(window.location.search);
    return urlParams.has("presentation");
